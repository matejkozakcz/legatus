-- ================================================================
-- Fix: sync_activity_from_meetings + přidat por_actual
--
-- Problémy které opravujeme:
-- 1. BJ se počítalo jen kde poradenstvi_status = 'probehle'
--    → nový formulář toto pole nenastavuje → BJ vždy 0
-- 2. POR schůzky se vůbec nepočítaly (sloupec por_actual chyběl)
-- 3. bj_fsa_actual nemělo smysl (FSA nemá podpisy)
-- ================================================================

-- 1. Přidat por_actual do activity_records
ALTER TABLE public.activity_records
  ADD COLUMN IF NOT EXISTS por_actual integer NOT NULL DEFAULT 0;

-- 2. Opravit sync funkci
CREATE OR REPLACE FUNCTION public.sync_activity_from_meetings(
  p_user_id    uuid,
  p_week_start date
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_fsa_actual  integer;
  v_por_actual  integer;
  v_ser_actual  integer;
  v_poh_actual  integer;
  v_bj          numeric;
  v_bj_por      numeric;
  v_bj_ser      numeric;
  v_ref_actual  integer;
BEGIN
  SELECT
    -- Počty schůzek podle typu (jen neodvolané)
    COUNT(*) FILTER (WHERE meeting_type = 'FSA' AND NOT cancelled),
    COUNT(*) FILTER (WHERE meeting_type = 'POR' AND NOT cancelled),
    COUNT(*) FILTER (WHERE meeting_type = 'SER' AND NOT cancelled),
    COUNT(*) FILTER (WHERE meeting_type = 'POH' AND NOT cancelled),
    -- BJ: jen POR a SER, přímo z podepsane_bj (bez podmínky poradenstvi_status)
    COALESCE(SUM(podepsane_bj) FILTER (WHERE meeting_type IN ('POR', 'SER') AND NOT cancelled), 0),
    COALESCE(SUM(podepsane_bj) FILTER (WHERE meeting_type = 'POR' AND NOT cancelled), 0),
    COALESCE(SUM(podepsane_bj) FILTER (WHERE meeting_type = 'SER' AND NOT cancelled), 0),
    -- Doporučení: součet všech typů
    COALESCE(SUM(doporuceni_fsa + doporuceni_poradenstvi + doporuceni_pohovor) FILTER (WHERE NOT cancelled), 0)
  INTO
    v_fsa_actual, v_por_actual, v_ser_actual, v_poh_actual,
    v_bj, v_bj_por, v_bj_ser,
    v_ref_actual
  FROM public.client_meetings
  WHERE user_id    = p_user_id
    AND week_start = p_week_start;

  INSERT INTO public.activity_records (
    user_id, week_start,
    fsa_actual, por_actual, ser_actual, poh_actual,
    bj, bj_fsa_actual, bj_ser_actual,
    ref_actual
  )
  VALUES (
    p_user_id, p_week_start,
    v_fsa_actual, v_por_actual, v_ser_actual, v_poh_actual,
    v_bj, v_bj_por, v_bj_ser,
    v_ref_actual
  )
  ON CONFLICT ON CONSTRAINT activity_records_user_week_unique
  DO UPDATE SET
    fsa_actual    = EXCLUDED.fsa_actual,
    por_actual    = EXCLUDED.por_actual,
    ser_actual    = EXCLUDED.ser_actual,
    poh_actual    = EXCLUDED.poh_actual,
    bj            = EXCLUDED.bj,
    bj_fsa_actual = EXCLUDED.bj_fsa_actual,
    bj_ser_actual = EXCLUDED.bj_ser_actual,
    ref_actual    = EXCLUDED.ref_actual,
    updated_at    = now();
END;
$$;
