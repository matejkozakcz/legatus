...
export default function Hledani() {
  ...
  const { data: results = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Search profiles (people in structure)
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <Search className="h-6 w-6" style={{ color: "var(--text-primary)" }} />
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          autoFocus
          className="w-full h-12 pl-12 pr-4 rounded-2xl border bg-card text-foreground font-body text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          style={{
                ...
                <div className="space-y-1">
                  {items.map((item) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-3 rounded-xl hover:bg-accent/60 transition-colors flex items-center gap-3 group"
                      style={{
                        background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,85,95,0.03)",
                      }}
                    >
