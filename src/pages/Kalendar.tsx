...
function MeetingDetailModal({ open, onClose, meeting, onEdit, onSaveOutcome, savingOutcome }: {
  ...
  // Sync form state when meeting changes
  useEffect(() => {
    if (meeting) {
  ...
  const showOutcome = !m.cancelled && m.date <= today;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-sm bg-card rounded-2xl shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-150 mx-4 overflow-y-auto"
        ...
        <button onClick={onEdit}
          className="w-full mt-4 h-10 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors hover:opacity-80 border border-input text-muted-foreground">
