...
const TABS = ["Profil", "Notifikace", "Kalendář", "gOWL"] as const;
...
export function SettingsModal({ open, onClose, initialTab = 0 }: SettingsModalProps) {
  ...
  const handleSaveProfile = async () => {
    ...
    if (showPassword && (newPassword || confirmPassword)) {
      if (newPassword.length < 8) { setPasswordError("Heslo musí mít alespoň 8 znaků"); return; }
  ...
  const renderProviderRow = (provider: "google" | "apple") => {
    ...
    return (
      <div key={provider} className="flex items-center justify-between py-2.5">
        ...
        {linked ? (
          <button onClick={() => handleUnlinkProvider(provider)} disabled={isUnlinking || identities.length <= 1}
          ...
          <button onClick={() => handleLinkProvider(provider)} disabled={!!linkingProvider}
  ...
  const selectClass = "h-9 rounded-xl border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring text-foreground";
  const inputClass = "w-full h-10 rounded-xl border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring text-foreground";
  ...
  const renderProfil = () => (
    <div className="space-y-5">
      ...
      <div className="flex flex-col items-center">
        <div className="relative w-20 h-20 rounded-full cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
          {uploading ? (
            <div className="w-20 h-20 rounded-full bg-muted flex items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>
          ) : avatarUrl ? (
            <img src={avatarUrl} alt="Avatar" className="w-20 h-20 rounded-full object-cover" />
          ) : (
            <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: "hsl(var(--deep))" }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input type="text" value={prijmeni} onChange={e => setPrijmeni(e.target.value)} className={inputClass} required />
        </div>
      </div>
      ...
      <button type="button" onClick={() => {
        ...
        if (showPassword) { setNewPassword(""); setConfirmPassword(""); setPasswordError(""); }
      ...
      {showPassword && (
        <div className="space-y-3">
          <div>
            ...
            <input type="password" value={newPassword} onChange={e => { setNewPassword(e.target.value); setPasswordError(""); }} placeholder="Min. 8 znaků" className={inputClass} />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Potvrdit heslo</label>
            <input type="password" value={confirmPassword} onChange={e => { setConfirmPassword(e.target.value); setPasswordError(""); }} className={inputClass} />
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{godMode ? "Admin pohled aktivní" : "Zobrazuji vlastní data"}</p>
              </div>
            </div>
            <button type="button" role="switch" aria-checked={godMode} onClick={toggleGodMode}
              className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      ...
      {/* Save */}
      <button onClick={handleSaveProfile} disabled={saving} className="btn btn-primary btn-md w-full flex items-center justify-center gap-2 mt-2">
  ...
  const renderNotifikace = () => (
    <div className="space-y-1">
      {/* Připomínka schůzky */}
      {renderToggleRow("Připomínka schůzky", notifPrefs.meetingReminder, v => updateNotif({ meetingReminder: v }),
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-xs text-muted-foreground mb-1">Kolikrát</label>
            <select value={notifPrefs.meetingReminderCount} onChange={e => updateNotif({ meetingReminderCount: Number(e.target.value) })} className={selectClass}>
              <option value={1}>1×</option><option value={2}>2×</option><option value={3}>3×</option>
            </select>
          </div>
          <div className="flex-1">
            <label className="block text-xs text-muted-foreground mb-1">Jak moc předem</label>
            <select value={notifPrefs.meetingReminderBefore} onChange={e => updateNotif({ meetingReminderBefore: e.target.value })} className={selectClass}>
              <option value="15min">15 min</option><option value="30min">30 min</option><option value="1h">1 hodinu</option><option value="1d">1 den</option><option value="2d">2 dny</option>
            </select>
          </div>
        </div>
      )}
      ...
      {/* Post-meeting prompt */}
      {renderToggleRow("Tak jak dopadla schůzka? 😎", notifPrefs.postMeeting, v => updateNotif({ postMeeting: v }),
        <div>
          <label className="block text-xs text-muted-foreground mb-1">Za jak dlouho po skončení</label>
          <select value={notifPrefs.postMeetingDelay} onChange={e => updateNotif({ postMeetingDelay: e.target.value })} className={selectClass}>
            <option value="ihned">Ihned</option><option value="30min">Za 30 minut</option><option value="1h">Za 1 hodinu</option>
          </select>
        </div>
      )}
      ...
      {/* Deadline */}
      {renderToggleRow("Deadline upozornění", notifPrefs.deadlineAlert, v => updateNotif({ deadlineAlert: v }),
        <>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Dní před koncem období</label>
            <input type="number" min={1} max={14} value={notifPrefs.deadlineDaysBefore}
              onChange={e => updateNotif({ deadlineDaysBefore: Math.max(1, Number(e.target.value)) })}
              className={`${inputClass} w-24`} />
          </div>
          <p className="text-xs text-muted-foreground italic">Pokud máš stanovený cíl, upozornění se přizpůsobí tvému plnění.</p>
        </>
      )}

      {/* Save */}
      <button onClick={handleSaveNotifications} className="btn btn-primary btn-md w-full flex items-center justify-center gap-2 mt-4">
        Uložit nastavení
      </button>
    </div>
  );
  ...
  const tabContent = [
    renderProfil,
    renderNotifikace,
    () => renderPlaceholder(<CalendarX className="h-10 w-10 text-muted-foreground" />, "Propojení s externím kalendářem bude dostupné brzy."),
    () => renderPlaceholder(<Puzzle className="h-10 w-10 text-muted-foreground" />, "Připravujeme propojení s gOWL."),
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0" style={{ background: isDark ? "rgba(0,0,0,0.6)" : "rgba(0,85,95,0.35)", backdropFilter: "blur(2px)" }} />
